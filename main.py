...
class VLCSimulator(QMainWindow):
    ...
    def init_ui(self):
        ...
        wm_layout.addWidget(QLabel("Parity:"), 2, 0)
        self.wm_parity_combo = QComboBox()
        self.wm_parity_combo.addItem("None", serial.PARITY_NONE)
        self.wm_parity_combo.addItem("Even", serial.PARITY_EVEN)
        self.wm_parity_combo.addItem("Odd", serial.PARITY_ODD)
        self.wm_parity_combo.addItem("Mark", serial.PARITY_MARK)
        self.wm_parity_combo.addItem("Space", serial.PARITY_SPACE)
        wm_layout.addWidget(self.wm_parity_combo, 2, 1)
        ...
        wm_layout.addWidget(self.wm_connect_btn, 4, 0, 1, 2)

        self.wm_status_label = QLabel("Status: Disconnected")
        self.wm_status_label.setStyleSheet("color: red;")
        wm_layout.addWidget(self.wm_status_label, 5, 0, 1, 2)

        self.wm_tx_label = QLabel("TX Count: 0")
        wm_layout.addWidget(self.wm_tx_label, 6, 0, 1, 2)
        ...
        ma_layout.addWidget(QLabel("Parity:"), 2, 0)
        self.ma_parity_combo = QComboBox()
        self.ma_parity_combo.addItem("None", serial.PARITY_NONE)
        self.ma_parity_combo.addItem("Even", serial.PARITY_EVEN)
        self.ma_parity_combo.addItem("Odd", serial.PARITY_ODD)
        self.ma_parity_combo.addItem("Mark", serial.PARITY_MARK)
        self.ma_parity_combo.addItem("Space", serial.PARITY_SPACE)
        ma_layout.addWidget(self.ma_parity_combo, 2, 1)

        self.ma_mode_label = QLabel("Mode: TIMEOUT")
        ma_layout.addWidget(self.ma_mode_label, 3, 0, 1, 2)

        self.ma_connect_btn = QPushButton("Connect MA")
        self.ma_connect_btn.clicked.connect(self.toggle_ma_connection)
        ma_layout.addWidget(self.ma_connect_btn, 4, 0, 1, 2)

        self.ma_status_label = QLabel("Status: Disconnected")
        self.ma_status_label.setStyleSheet("color: red;")
        ma_layout.addWidget(self.ma_status_label, 5, 0, 1, 2)

        self.ma_tx_label = QLabel("TX Count: 0")
        ma_layout.addWidget(self.ma_tx_label, 6, 0, 1, 2)
    ...
    def toggle_wm_connection(self):
        ...
        if self.wm_serial and self.wm_serial.is_open:
            ...
            try:
                ...
                parity = self.wm_parity_combo.currentData()
                parity_name = self.wm_parity_combo.currentText()
                self.wm_serial = serial.Serial(port, baud, parity=parity, timeout=0.1)
                self.wm_connect_btn.setText("Disconnect WM")
                self.wm_status_label.setText(f"Status: Connected ({port} @ {baud}, {parity_name})")
    ...
    def toggle_ma_connection(self):
        ...
        if self.ma_serial and self.ma_serial.is_open:
            ...
            try:
                ...
                parity = self.ma_parity_combo.currentData()
                parity_name = self.ma_parity_combo.currentText()
                self.ma_serial = serial.Serial(port, baud, parity=parity, timeout=0.1)
                self.ma_connect_btn.setText("Disconnect MA")
                self.ma_status_label.setText(f"Status: Connected ({port} @ {baud}, {parity_name})")
